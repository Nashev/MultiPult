...
type
  TControlAction = (caNone, caStepPrev, caStepNext, caPlayBackward, caPlayForward);
  TFrame = class
    FileName: string;
...
begin
  if ssCtrl in Shift then
    ...
    if not KeyPressBlocked then
      case Key of
        vk_Left:  begin PushControlAction(caStepPrev); KeyPressBlocked := True; end;
        vk_Right: begin PushControlAction(caStepNext); KeyPressBlocked := True; end;
      end;
...
begin
  if Integer(FileNames.Objects[Index]) = 0 then
    LoadPhoto(Index);
  CurrentFrameIndex := Index;
  if not Exporting then
    SetCaption(FileNames[CurrentFrameIndex]);
  pbDisplay.Repaint;
  pbTimeLine.Repaint;
end;
...
var
  ...
  Frame: TFrame;
begin
  if Assigned(Frames[Index].OriginalJpeg) then
    Exit;
  ...
  Frames[Index].OriginalJpeg := Image;
end;
...
begin
  if Frames[CurrentFrameIndex] = TMenuItem(Sender).Tag then
    Teleports(CurrentFrameIndex) := -1
  else
    Teleports(CurrentFrameIndex) := TMenuItem(Sender).Tag;
end;
...
var
  ...
  Image: TJPEGImage;
begin
  if CurrentFrameIndex < FileNames.Count then
    begin
      Image := TFrame(Frames[Integer(FileNames.Objects[CurrentFrameIndex])]).OriginalJpeg;
      r.Left :=  0;
      r.Top := 0;
...
begin
  ...
  for y := 0 to pbRecord.Height do
    if y < RecordedFrames.Count then
      pbRecord.Canvas.Pixels[MulDiv(Integer(RecordedFrames[RecordedFrames.Count - 1 - y]), pbRecord.Width, FileNames.Count), y] := clBlack
    else
      Break;
  if Playing then
    if CurrentRecordPosition < RecordedFrames.Count then
      with Point(MulDiv(Integer(RecordedFrames[CurrentRecordPosition]), pbRecord.Width, FileNames.Count), RecordedFrames.Count - 1 - CurrentRecordPosition) do
...
begin
  ...
  for SecondIndex := 0 To FileNames.Count * CurrentSpeedInterval div FrameRate - 1 do
    begin
      x := 4 + MulDiv(pbTimeLine.Width - 8, SecondIndex, FileNames.Count * CurrentSpeedInterval div FrameRate);
      pbTimeLine.Canvas.Brush.Style := bsSolid;
      pbTimeLine.Canvas.Rectangle(x, y1 + 8, x+1, y1 + 16);
    end;

  for FrameIndex := 0 to FileNames.Count - 1 do
    begin
      x := 4 + MulDiv(pbTimeLine.Width - 8, FrameIndex, FileNames.Count);
      if Integer(FileNames.Objects[FrameIndex]) = 0 then
        y2 := y1 + 4
      else
        y2 := y1 + 8;
  ...
  if Playing then
    ...
    begin
      ...
      if CurrentFrameIndex < 0 then
        CurrentFrameIndex := FileNames.Count - 1;
      if CurrentFrameIndex > (FileNames.Count - 1) then
        CurrentFrameIndex := 0;
...
begin
  TAction(Sender).Enabled := not Exporting and (FileNames.Count > 0);
end;
...
begin
  ...
  for i := 0 to FileNames.Count-1 do
    if Integer(FileNames.Objects[i]) = 0 then
      begin
        LoadPhoto(i);
        pbTimeLine.Repaint;
        Done := False;
