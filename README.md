...
Исходный код программы доступен для просмотра и доработок на [github](https://github.com/Nashev/MultiPult). Так же используется сервис Waffle.io? чтобы отмечать, какие из задач в работе и сколько задач достаточно продумано, чтобы к ним можно было приступить в свободную минутку [![Issues in Ready for work](https://badge.waffle.io/nashev/multipult.png?label=ready to work&title=Ready), [![Issues in progress](https://badge.waffle.io/nashev/multipult.png?label=in progress&title=InProgress)](http://waffle.io/nashev/multipult)
